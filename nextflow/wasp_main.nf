...
process allignment_hisat2_1 {
    ...
    output:
    tuple val(species), val(sid), file("${sid}1.sorted.bam"), file("${sid}1.sorted.bam.bai"), file('hisat_index') into hisat1
	...
	-2 ${species}_${sid}_reverse_paired.fastq.gz \
        ...
        --threads 16 \
            | samtools view -bS -F 4 -F 8 -F 256 - > ${sid}1.bam
   
    samtools sort ${sid}1.bam -o ${sid}1.sorted.bam
    samtools index ${sid}1.sorted.bam
    """
}


process allignment_hisat2_2 {
    cache 'lenient'
    errorStrategy 'finish'
    tag {'allign_' + '_' + sid }
    ...
    input:
    tuple val(species), val(sid), file("${sid}1.sorted.bam"), file("${sid}1.sorted.bam.bai"), file('hisat_index') from hisat1
    ...
    python /home/bop20pp/software/WASP-master/mapping/find_intersecting_snps.py \
	--is_paired_end \
	--is_sorted \
        --output_dir find_intersecting_snps \
	--snp_dir /fastdata/bop20pp/supergene_AB_maleref/wasp/snp_files/${sid} \
	${sid}1.sorted.bam
    mkdir map1 map2 filter_remapped_reads
    

    hisat2 \
	-x hisat_index/index \
        -1 find_intersecting_snps/${sid}1.sorted.remap.fq1.gz \
        -2 find_intersecting_snps/${sid}1.sorted.remap.fq2.gz \
        --threads 16 \
        --mp 10,2 \
           | samtools view -bS -F 4 -F 8 -F 256 - > map2/${sid}2.bam
    samtools sort map2/${sid}2.bam -o map2/${sid}2.sorted.bam
    samtools index map2/${sid}2.sorted.bam



    python /home/bop20pp/software/WASP-master/mapping/filter_remapped_reads.py \
	find_intersecting_snps/${sid}1.sorted.to.remap.bam \
	map2/${sid}2.sorted.bam \
	filter_remapped_reads/${sid}.keep.bam


    mkdir merge 
    samtools merge merge/${sid}.keep.merge.bam \
              filter_remapped_reads/${sid}.keep.bam  \
              find_intersecting_snps/${sid}1.sorted.keep.bam
    samtools sort -o  merge/${sid}.keep.merge.sort.bam \
              merge/${sid}.keep.merge.bam 
    samtools index merge/${sid}.keep.merge.sort.bam

    python /home/bop20pp/software/WASP-master/mapping/rmdup_pe.py merge/${sid}.keep.merge.sort.bam ${sid}.bam
    """
...
process RG_add {
    ...
    samtools sort -o ${sid}.sorted.bam ${sid}.bam 
    mkdir `pwd`/tmp
    picard AddOrReplaceReadGroups I=${sid}.sorted.bam O=${sid}.RG.bam SORT_ORDER=coordinate RGID=zf RGLB=zf RGPL=illumina RGSM=zf RGPU=zf CREATE_INDEX=True TMP_DIR=`pwd`/tmp
    """
...
process genotpye_gvcfs_ASE {
    ...
    tag {'genotpye_gvcfs'}
    publishDir 'wasp_genotyped_vcfs', mode: 'copy', overwrite: true, pattern: '*vcf.gz'
